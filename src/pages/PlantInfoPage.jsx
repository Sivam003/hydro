...
const plantCareData = {
  ...
  "Cabbage": { sunlight: "6+ hours", pH: "6.5-7.0", temp: "15-24°C", ec: "2.5-3.0 mS/cm", fertilizer: "Balanced with sufficient Nitrogen" },
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
              {plant.photo && (
                <img src={plant.photo} alt={plant.name} className="w-full h-auto max-h-80 object-cover rounded-lg shadow-md mb-4" />
              )}
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-96 overflow-y-auto p-2 border rounded-md">
                            {galleryImages.sort((a,b) => new Date(b.date) - new Date(a.date)).map((img) => (
                              <div key={img.id} className="relative group">
                                <img src={img.url} alt={`Growth on ${formatDate(img.date)}`} className="w-full h-32 object-cover rounded-md" />
